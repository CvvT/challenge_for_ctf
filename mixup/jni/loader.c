...
void decryptAndcall(void *stage, int size) {
    unsigned int base, npage;
    void (*fct)();
    long page_size = sysconf(_SC_PAGESIZE);
    base = (unsigned int)(stage - 1);

	fct = stage;
    npage = size / page_size + ((size % page_size == 0) ? 0 : 1);
    npage = size / page_size + ((size % page_size == 0) ? 0 : 1);
    if(mprotect((void *) (base / page_size * page_size), npage, PROT_READ | PROT_EXEC | PROT_WRITE) != 0){
        return;
    }
    Notdecrypt(base, size);
    if(mprotect((void *) (base / page_size * page_size), npage, PROT_READ | PROT_EXEC) != 0){
    	return;
    }
